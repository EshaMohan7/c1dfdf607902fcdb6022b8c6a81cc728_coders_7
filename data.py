...
def generate_random_phone_number():
    ...
    line_number = np.random.randint(1000, 9999)
...
customer_data = pd.DataFrame({
    ...
    'Phone_No': [generate_random_phone_number() for i in range (1000)]
})
...
for index, row in df.iterrows():
    folium.Marker(
        ...
        popup=f"{row['MerchantName']} - {row['Location']}",
        icon=folium.Icon(color='blue')
    ).add_to(mymap)

# Save the map to an HTML file
mymap.save("merchant_locations_map.html")